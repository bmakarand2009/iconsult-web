  ...
  function ($document, $compile, $rootScope, $controller, $timeout) {
      ...
      return function Dialog(templateUrl/*optional*/, options, passedInLocals) {
          ...
          var defaultFooter = '<button class="btn" ng-click="$modalCancel()">{{$modalCancelLabel}}</button>' +
            '<button type="submit" class="btn btn-primary">{{$modalSuccessLabel}}</button>';
          ...
          var modalEl = angular.element(
              '<div class="' + options.modalClass + ' fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"' + idAttr + '>' +
              ...
              '      </div>' +
              '      <form ng-submit="submit($modalSuccess)" editable-form>' +
              modalBody +
              footerTemplate +
              '      </form>' +
              '    </div>' +
